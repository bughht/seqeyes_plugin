...
function drawRfOverview(summary,vi,ch,colors,vs,ve){
  ...
  rowClip(vi,ch,function(){
    ...
    ctx.globalAlpha=.55;ctx.fillStyle=colors.rf;
    for(var peakBin=0;peakBin<bins.peak.length;peakBin++){
      if(bins.events[peakBin]<1||bins.peak[peakBin]<=0)continue;
      var peakX=t2x(vs+(peakBin+.5)*bins.secondsPerPixel);ctx.fillRect(peakX-.5,base-bins.peak[peakBin]*scale-.5,1,1);rfOverviewBucketCount++;rfRenderPointCount++;
    }
    ctx.globalAlpha=1;

/**
 * Webview HTML for SeqEyes.
 *
 * Renders an interactive Canvas‑based sequence diagram inside a VS Code
 * webview panel.  Block data arrives via `postMessage({ type:'sequenceData' })`.
 *
 * Features
 *   • 7 toggle‑able channels: RF, φ, Gx, Gy, Gz, ADC, Trigger
 *   • Zoom (scroll) & pan (drag)
 *   • Vertical cursor with live time readout
 *   • Y‑axis labels with physical units
 *   • Unit switchers for time (s/ms/µs) and gradient (Hz/m, mT/m, G/cm)
 *   • Trigger arrows, ADC phase indicator, frequency‑modulated RF phase
 */
...
export function getWebviewContent(_hint: number): string {
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>SeqEyes</title>
<style>/* ── Colour scheme ── */
...
var BL=[],TD=0;                            // block data & total duration
var M={t:8,r:30,b:22,l:92};                // margins
var CH=['RF','\u03c6','Gx','Gy','Gz','ADC','Trig'];               // channel names
...
var gMax=[1,6.28318,1,1,1,0,0];          // global max per channel (Hz, rad, Hz/m)
...
window.addEventListener('message',function(e){
  var m=e.data;if(m.type==='sequenceData'){BL=m.blocks||[];TD=m.totalDuration||0;computeGlobalMax();fit();draw();}
});
...
function drawBlocks(vs,ve,s){
  ...
  for(var bi=0;bi<BL.length;bi++){
    ...
    // ── RF magnitude (0 at bottom, fills upward) ──
    if(b.rf&&viRF>=0){
    ...
    // ── RF phase (0 at bottom, 2π at top) ──
    ...
    // ── Triggers (downward triangle arrows) ──
    if(b.trg&&viTrig>=0){
